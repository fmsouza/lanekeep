...
## In an editor, and under `--watch`

`lanekeep server` holds one type provider for the length of a session and rebuilds the engine for each request. What that buys is the declaration cache: a library's `.d.ts` is parsed once for the session rather than once per save, and under `types.provider: 'tsc'` the sidecar's program survives too.

What it does not buy is a different answer. Under `builtin` the provider re-hashes each declaration it holds before every request and drops any whose bytes moved; it memoizes no absences at all — an absent declaration is a null-hashed tracked read on the *importing file's* cache entry, and the entry is what notices when it appears — and it forgets its per-file `complete()` verdicts wholesale, because a `bool` carries no hash to compare against. Under `tsc` the held state is the sidecar's programs, and beginning a run re-answers `programs`: everything each program read is re-hashed, and every path its build was denied is re-probed, so a package installed since the last request rebuilds the program that resolution had fallen back from. Either way a session that has been open all afternoon answers exactly what `lanekeep check` answers over the same tree.

If the sidecar is *gone* rather than stale — a breached `timeouts.analysis` kills it — the session builds a new provider for the next request rather than failing every request from then on. And a session that cannot build one at all does not fail the request: it hands the engine none and lets the engine decide, which is how `lanekeep server` answers exactly the requests `lanekeep check` answers when `types.provider: 'tsc'` names a command that will not start and no enabled rule needs `types`. `crates/lanekeep-cli/tests/server_agreement.rs` asserts that equality directly and under both providers: it opens two files, compares the published diagnostics with `lanekeep check --format json`, edits a declaration file on disk, sends `didSave`, and compares again.

If the `types` block in your config changes while the session is open, the provider is rebuilt rather than revalidated. A provider built from a different configuration is a different answer, not a stale one.

`--watch` holds nothing: each iteration builds a fresh engine and a fresh provider, so the twentieth is byte-identical to a cold run — `crates/lanekeep-cli/tests/watch_iterations.rs` drives twenty `lanekeep check` runs against one cache directory, the first cold, which is that sequence with nothing in the way of observing it. It does watch for declaration files, though. After each iteration the paths the run recorded as tracked reads become an allowlist, and an event on one of them wakes the loop even under `node_modules` — which is where a Node project's declaration files are, and which the watcher otherwise ignores wholesale. The grant is per path rather than per directory, and it is compared against the path *relative to the project root* rather than by trailing components, so installing a package does not put the event storm back — a single-component entry like `tsconfig.json`, which every `tsc` listing carries, matched every `node_modules/*/tsconfig.json` in the tree while the comparison was by tail. An iteration that *fails* clears the set rather than keeping the last good one: it read some unknown prefix of a run and cannot claim to describe the inputs, so the loop wakes for anything outside an ignored directory until a run succeeds.
