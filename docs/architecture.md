...
**Where each capture binds is part of the contract, not a matter of taste.** `@sanitizer` is the whole call — `(call_expression function: (identifier) @fn (#eq? @fn "redact")) @sanitizer` — because the analyzer cuts a flow only where the sanitizer node *is* the value read at the sink or *contains* the source, and a call is the only node that is both. `@sink` is the value that must not arrive — an argument, a template substitution, an assignment's right-hand side — never the call around it, since a call is opaque to the analysis and would report only the direct `log(getSecret())` shape. `@source` may sit on the call or on its callee identifier, because a source is found by containment in the expression the sink reads and either node is inside it. The natural misspelling, `@sanitizer` or `@sink` on the callee identifier, compiles, matches and does nothing: the shipped `lanekeep/no-secret-in-string` carried it from the day it landed until #222, its documented-silent `log(redact(getSecret()))` reporting the whole time. Config load now refuses a `@sanitizer` or `@sink` bound in a call's callee slot — `function:`, or `constructor:` on a `new` — naming the rule and the remedy (#223). The check is lexical: neither `lanekeep-config` nor tree-sitter's compiled `Query` can ask a capture which node it binds, so `lanekeep-query`'s `capture_sites` reads the slot off the query's text, with no grammar in hand. It refuses the shape that shipped rather than every inert shape — a sanitizer bound inside a member callee's receiver is as inert and is not caught.
