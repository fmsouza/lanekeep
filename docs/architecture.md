...
**It builds outside every gate, and the artifact is a build output rather than a committed file.** `just typescript-builtins` needs Node, and `just check` must pass on a machine that has none — so the component reaches the binary through `include_bytes!` after being built by `build.rs` (or by the CI build job) into `crates/lanekeep-rules/components/`, exactly as the Rust ones do. `componentize-js` is **not** byte-reproducible (two builds of one unchanged tree differ in ~2.9 MB), so there is no "rebuild and see whether `git status` is clean" check — the artifact is produced fresh by the build rather than compared against a committed copy.
...
5. **A clean internal `Rule` boundary.** Built-in rules are authored against the same host API as user rules — the strongest available evidence that the API is sufficient. A built-in needing to be reimplemented in another language does so behind the same ID and the same trait.
    ...
    **Two of those four test files do not execute the component, and the claim is worth stating at the strength it holds.** `lanekeep-cli/tests/no_circular_imports.rs` and `.../no_unused_exports.rs` drive the real binary, so they run the artifact. `lanekeep-rules/tests/no_default_export.rs` and `.../no_restricted_imports.rs` ask for the rule's authored TypeScript and run *that* in QuickJS — so what they establish is that the source still passes, which was not in doubt, rather than that the compiled form agrees with it. What holds the other half is `lanekeep-rules/tests/typescript_builtins_as_components.rs`, added afterward: it re-encodes enough of those two files' cases against the shipped component to catch a different program behind the same id. Unmodified test files were the right acceptance test and they are not the whole of the evidence.
