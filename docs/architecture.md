...
Its shape is a floor plus a small marginal cost. Reproduce the marginal figure by dropping the second row of `go-rules/main.go`'s `ruleset` and rebuilding: one rule is **9,941 bytes** and two are **13,207**, so **a Go rule costs about 3,266 bytes** after the first. Both halves of that subtraction have to come from one build of one SDK, which is the trap this paragraph walked into once already: it read 13,187 − 9,941 = 3,246, taking the two figures from commits with an SDK change between them — the same "compare two binaries over one corpus, never one binary over two trees" that AGENTS.md records, in miniature. 13,187 is what the two-rule artifact came out at *after* that SDK change and is the right figure for the sentence above; it is the wrong one for this subtraction.

Two things follow. The component-count arithmetic for Go is Rust's rather than JavaScript's — one component per rule would be perfectly affordable at 13 KB each — and the reason both Go rules share one anyway is that most of those bytes are the TinyGo runtime, which a second artifact would duplicate for nothing. And **the figures in the two paragraphs above do not apply to it**, which is a claim about the class rather than about the number: those were measured against a component of about 26 KB, the Rust ones ship at ~108 KB today, and the Go one is 13 KB — all three within a factor of ten of each other and all three about a thousandth of the JavaScript component. It is that last ratio the two paragraphs above turn on.
...
**That is a judgment rather than a hard limit, and it used to be written here as a hard limit — which invites someone to delete the 13 MB artifact that *is* committed.** crates.io's 10 MiB cap is on the compressed package, and `lanekeep-rules` ships `typescript-builtins.wasm` at 13,029,888 bytes inside a package that measures 4,336,984 bytes. So one JavaScript component fits, comfortably; the reasons this particular one stays out are that it is a benchmark input rather than something the crate ships, that `componentize-js` is not byte-reproducible so nothing could establish a committed copy was current (§5.2), and that the ~4 MiB of the cap already spent belongs to artifacts users run.
