...
`scope: 'function'` asks whether every path out of the enclosing function — `return` and `throw` included — passes a release; `scope: 'block'` narrows the question to the lexical block the acquire sits in. `scope: 'module'` asks a different kind of question: whether a release exists anywhere in the file, correlated to the same value by `@key` (below) — there is no path to walk, because sibling functions share no control-flow graph at all. `unmet.exit` is the source-earliest `return`, `throw`, or implicit function end reachable from the acquire without crossing a release, and `unmet.partial` says whether *some* path did discharge it — a resource zeroed on the happy path but missed on an early `return` reads differently in a report than one never zeroed at all. Neither has a path to report under `'module'`: `unmet.exit` is the acquire node itself, and `unmet.partial` is always `false`.

**Per-file always, intra-procedural except under `scope: 'module'`.** Nothing crosses a function boundary under `'function'`/`'block'` scope (a nested function is opaque to its parent's graph, §3) — `'module'` is the deliberate exception: `@key` correlation lets an acquire in one function discharge against a release in a sibling function, no control-flow graph shared between the two at all — the register/forget pattern `docs/obligation-rules.md` works through in full. Nothing crosses a file boundary regardless of scope, `'module'` included — it is file-wide, not cross-file — so `checkObligation` runs from the engine's ordinary per-file dispatch, in the same `check` phase as the main query loop and under the same per-invocation timeout, with its `ctx.report` calls harvested exactly as `check`'s are. That has a consequence worth stating plainly: **unlike a cross-file `reduce` rule (§4, "Cross-file rules"), an obligation rule is never skipped by `--since`, `--staged` or `--file`** (§8.4) — there is no whole-corpus fact set for a subset run to get wrong, so checking three changed files is exactly as sound as checking all of them. It is the one capability in this document that gets *more* useful in the pre-commit loop rather than less.
