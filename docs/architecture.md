...
Both answer from the parsed file alone — no `tsconfig.json`, no declaration files, no cross-file resolution — and both return `undefined` freely. Nor do they answer anything type-level the file itself computes: no generic instantiation (`useQuery<Balance[]>` is reachable by its constructor's name, never by its argument), no conditional or mapped types, no declaration merging — each is `undefined`. That is survivable for a rule that needs a primitive or a nominal name, and it is not survivable for a rule over a type-level library, so a type-aware rule's documentation states which it is. **`undefined` is the oracle's first-class answer, not a failure to work around.** It would rather say nothing than say something wrong: a rule reporting on a wrong type accuses correct code, which is a worse failure than reporting on nothing. A rule is expected to check for `undefined` and quietly stay silent, the same posture the rest of this section already takes on a dead handle (§6.2).

The cost of that posture is that a type-aware rule is silent for three different reasons and its report shows none of them: the gate rejected the file (§7.1), the oracle answered `undefined`, or the code is clean. `--profile`'s gate table (§7.1) separates the first from the other two. Nothing separates the second from the third — an oracle-coverage column is open question 3 of #185 §A.7 — which is why the built-in rules' documentation lists what each stays silent on rather than leaving a clean report to mean conformance.
