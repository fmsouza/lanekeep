...
| Field-sensitive | **no** | `o.secret = s` taints `o` entirely; reading `o.public` after is tainted |
| Index-sensitive | **no** | `a[0] = s` taints `a` |
| Alias-aware | **local only** | `const b = a` propagates; aliasing through a call does not |
| Context-sensitive | **n/a** | intra-procedural |

Flow-sensitivity is the only dimension v1 commits to unconditionally. Of the rest, field-sensitivity is named specifically, because it is the one that costs a real project real false positives: mutating one field of an object is common enough that tainting the whole binding — and every other field read from it afterward — shows up in practice, not only in a contrived fixture. It stays deferred rather than fixed: doing better needs taint tracked per access path (`o.secret` distinctly from `o.public`, rather than `o` as a whole) bounded by a widening, so a deep or cyclic object graph cannot make the analysis fail to terminate — an abstraction materially larger than the rest of this analysis combined. The other "no"s are ordinary v1 scoping rather than a named backlog item: path-, index- and context-sensitivity are out of scope the same way inter-procedural flow is, with no claim that any one of them is next.
