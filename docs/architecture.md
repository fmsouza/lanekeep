...
`requires: ['dataflow']` is the sibling capability, and it no longer refuses at load: #193 shipped an analysis for it, the obligation typestate check documented at §6.11. Both capabilities share the refusal posture — a language with no analyzer behind the token fails loudly rather than silently reporting nothing, the same reasoning §8.4 states for skipping a cross-file rule rather than running it wrong — they just differ in what the token unlocks: a `ctx.*` namespace for `types`, a second handler invocation (`checkObligation`) for `dataflow`.
...
**`--fix` does not apply.** `checkObligation` reports with no `Fix` (§10.1) — the remedy is almost always adding a `finally` that does not exist at the report site, not replacing the bytes at the node the violation is attached to, and the byte-range fix model has no way to express "insert a block here." `card.remediation` is expected to carry the whole remedy in prose, the same posture a reduce-phase violation already takes for the same reason.
...
An obligation violation (§6.11) carries no fix either, for the opposite reason: the tree is right there, but the remedy is almost never a replacement at the report site — it is adding a `finally` that does not exist yet. `checkObligation` has no `Fix` parameter to offer one through; `card.remediation` carries the whole remedy instead.
