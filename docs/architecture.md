...
- **Value identity is opt-in, through an optional `@key` capture (#248).** An acquire or release query may bind `@key` alongside its `@acquire`/`@release` capture; correlation is then exact captured-text equality — an acquire keyed `K` is discharged only by a release also keyed `K`, compared byte-for-byte against the source, never by dataflow or binding identity. Binding is all-or-nothing per obligation: either every acquire *and* release query binds `@key`, or none does, refused at load otherwise (below) — a rule keying half its queries has decided correlation matters for part of the obligation and not the rest, which is not a coherent shape to run. A match whose keyed query happens not to capture `@key` (an optional pattern fragment around it) has no key to correlate with anything and is always reported, the same safe-over-report posture as everywhere else in this section. `@key` correlation together with `scope: 'module'`, `scope: 'class'` or `scope: 'component'` above is the value-identity capability in full; there is no other mechanism.
