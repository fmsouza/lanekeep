...
`scope: 'function'` asks whether every path out of the enclosing function — `return` and `throw` included — passes a release; `scope: 'block'` narrows the question to the lexical block the acquire sits in. `scope: 'module'` and `scope: 'class'` (#258) each ask a different kind of question: whether a release exists somewhere, correlated to the same value by `@key` (below) — `'module'` searches the whole file, `'class'` bounds the search to the acquire's enclosing class, a sibling method's release included. Neither walks a control-flow graph, because sibling functions — or sibling methods of one class — share none. An acquire with no enclosing class at all is unconditionally reported under `'class'`: with no region to search, there is nothing to bound existence to, however the file's releases are keyed. `unmet.exit` is the source-earliest `return`, `throw`, or implicit function end reachable from the acquire without crossing a release, and `unmet.partial` says whether *some* path did discharge it — a resource zeroed on the happy path but missed on an early `return` reads differently in a report than one never zeroed at all. Neither has a path to report under `'module'` or `'class'`: `unmet.exit` is the acquire node itself, and `unmet.partial` is always `false`.

**Per-file always, intra-procedural except under `scope: 'module'` or `scope: 'class'`.** Nothing crosses a function boundary under `'function'`/`'block'` scope (a nested function is opaque to its parent's graph, §3) — `'module'` and `'class'` are the deliberate exceptions: `@key` correlation lets an acquire in one function or method discharge against a release in a sibling one, no control-flow graph shared between the two at all — the register/forget pattern `docs/obligation-rules.md` works through in full, for both scopes. `'class'` still stops at the class body: a release in a different class, or an acquire with no enclosing class at all, is not discharged, which is the sense in which it is narrower than `'module'` rather than a second file-wide scope under another name. Nothing crosses a file boundary regardless of scope, `'module'` and `'class'` included — both are within-file, not cross-file — so `checkObligation` runs from the engine's ordinary per-file dispatch, in the same `check` phase as the main query loop and under the same per-invocation timeout, with its `ctx.report` calls harvested exactly as `check`'s are. That has a consequence worth stating plainly: **unlike a cross-file `reduce` rule (§4, "Cross-file rules"), an obligation rule is never skipped by `--since`, `--staged` or `--file`** (§8.4) — there is no whole-corpus fact set for a subset run to get wrong, so checking three changed files is exactly as sound as checking all of them. It is the one capability in this document that gets *more* useful in the pre-commit loop rather than less.
