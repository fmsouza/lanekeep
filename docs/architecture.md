...
Beyond that, the builtin provider answers nothing type-level the program computes: no generic instantiation (`useQuery<Balance[]>` is reachable by its result type's name, never by its argument), no conditional or mapped types, no declaration merging, and no `.tsx` file reached through an import — the provider parses everything it opens with the TypeScript grammar, under which JSX produces a tree full of silent `ERROR` nodes that is worse than reading nothing. A declaration file is never TSX, so nothing is lost in `node_modules`; a **project source** is: `import { Button } from './Button'` with `Button.tsx` beside it resolves to nothing, records six absent reads and makes the importer `complete() === false`, which on a React codebase is most sibling imports. A stated limitation of this milestone rather than a property of the design, filed with the resolver's own issue. That grammar is chosen by name rather than by taking the first registered language that answers: `tsx` sorts before `typescript`, so the first-that-answers rule would parse every `.ts` file the provider opens with the wrong dialect. Each is `undefined`. That is survivable for a rule that needs a primitive, a nominal name or an inheritance answer, and it is not survivable for a rule over a type-level library, so a type-aware rule's documentation states which it is.
