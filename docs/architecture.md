...
**A TinyGo component sits at the other end of that range, and its size is the third one worth writing down.** `go-builtins.wasm` hosts both Go built-ins in **13,187 bytes** — `git cat-file -s` on the committed artifact at `2ce7aef`, built by `just go-rules` on TinyGo 0.41.1 with `-target=wasm-unknown -panic=trap -no-debug` — against 107,914 and 111,759 bytes for the two Rust rule components and 13,029,888 for the shared JavaScript one. Its shape is a floor plus a small marginal cost: the one-rule build at `e4845d7` was 9,941 bytes, so the second rule cost **3,246**. Two things follow. The component-count arithmetic for Go is Rust's rather than JavaScript's — one component per rule would be perfectly affordable — and the reason both Go rules share one anyway is that most of those bytes are the TinyGo runtime, which a second artifact would duplicate for nothing. And **the figures in the two paragraphs above do not apply to it**: they were measured against components of about 26 KB, which is the class a Go component is in, not the class the JavaScript one is in.

For scale at the other end, and read as magnitudes rather than as a ratio because the two were not taken on one profile: `crates/lanekeep-rules/tests/no_context_in_struct.rs` runs its whole eight-case table against the committed Go component in 0.12 s on the dev profile, where `crates/lanekeep-rules/tests/typescript_builtins_as_components.rs`'s header records ~6.5 s for a *single* first `run` against the JavaScript one on the release profile.
