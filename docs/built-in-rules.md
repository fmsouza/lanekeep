...
| `const o = { secret: getSecret() }; log(o.public);` | yes — documented over-approximation | a source *contained* in a literal taints every path asked of it (the containment scan answers before the path is consulted); a source reaching the literal through a *binding* is field-sensitive, as the rows below show |
| `const s = getSecret(); log({ cause: s });` | yes | a literal carries the union of its members; a tainted field taints the object read whole |
| `const s = getSecret(); const o = { cause: s }; log(o.cause);` | yes, but `log(o.public)` is **silent** | field-sensitive on the literal: a member is tainted only at the path it was written to |
| `const s = getSecret(); log({ s });` | yes | a shorthand property `{ s }` is `{ s: s }`, resolved as a reference to the binding |
| `const s = getSecret(); log([s]);` | yes | an array element taints the array; index-insensitive, so any element read reports |
| `const s = getSecret(); log(c ? s : x);` | yes | a conditional carries the union of its branches; a `@sanitizer` still cuts the branch it wraps |
| `const s = getSecret(); log((s)); log(s!); log(s as T); log(s satisfies T); log(await s);` | yes | transparent wrappers — parentheses, `!`, `as`, `satisfies`, `await` — pass the value through unchanged (there is no promise model, so `await` is transparent too) |
| `const o = { token: s }; log((o as T).token);` | yes | a transparent wrapper is peeled as a member *base* too, so `(config as Secrets).apiKey` is followed like `config.apiKey` |
| `const s = getSecret(); log({ cause: s }.cause);` | **no** — documented v1 false negative | an object or array literal used *directly* as a member base has no binding to attach to; bind it first (`const o = { cause: s }; log(o.cause)` reports) |
