...
/// no longer a built-in.** `lanekeep/no-unwrap` resolves to the component now. This arm is what
/// holds the swap to having changed nothing: it runs the implementation that was replaced,
/// against the same table, in the same commit that replaced it. It leaves with the file.
fn typescript(case: &Case) -> RuleTester {
    let source = include_str!("../rules/no-unwrap.ts");
    match case.options {
