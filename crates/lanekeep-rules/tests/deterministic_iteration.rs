//! `local/deterministic-iteration`, run through the real engine.
//!
...
//! that does not fit the table — `an_empty_iteration_scope_is_refused`, which asserts a *load*
//! failure rather than a set of violations — is a `#[test]` of its own at the bottom, modeled
//! on the selfcheck.rs original it was migrated from.
...
const RULE_ID: &str = "local/deterministic-iteration";
...
const REMEDIATION: &str =
    "use `BTreeMap` or `BTreeSet` — the order this iterates in becomes report order or cache bytes";
...
///
/// Migrated from `crates/lanekeep-cli/tests/selfcheck.rs`'s `iteration` group. The harness
/// case there — the `iteration()` helper that built a tester — was scaffolding and is not
/// migrated; the four assertions it served are. The fifth, `an_empty_iteration_scope_is_refused`,
/// asserts a load failure rather than a set of violations and lives as a `#[test]` of its own
/// below.
const CASES: &[Case] = &[
    // Iteration order here becomes report order or cache bytes.
    Case {
        name: "an_unordered_import_is_reported",
        source: "use std::collections::HashMap;\n",
        options: Some(r#"{"scope": ["subject/"]}"#),
        expected: Expected::ReportsAt(&[(1, 1)]),
    },
    // The type is the decision, wherever it is written.
    Case {
        name: "an_unordered_annotation_is_reported",
        source: "fn go(m: HashSet<u8>) {}\n",
        options: Some(r#"{"scope": ["subject/"]}"#),
        expected: Expected::ReportsAt(&[(1, 10)]),
    },
    // BTreeMap and BTreeSet iterate in key order.
    Case {
        name: "an_ordered_collection_passes",
        source: "use std::collections::BTreeMap;\nfn go(m: BTreeSet<u8>) {}\n",
        options: Some(r#"{"scope": ["subject/"]}"#),
        expected: Expected::Accepts,
    },
    // A HashMap whose order never leaves its crate is fine.
    Case {
        name: "a_file_outside_the_iteration_scope_passes",
        source: "use std::collections::HashMap;\n",
        options: Some(r#"{"scope": ["subject/other/"]}"#),
        expected: Expected::Accepts,
    },
];
...
/// `id: "local/deterministic-iteration"`, `severity: "warn"` and a remediation reading
...
/// The rule: a WebAssembly component built from `rust-rules/deterministic-iteration/`.
///
...
fn component(case: &Case) -> Option<RuleTester> {
    let component = component_bytes("deterministic-iteration")?;
    Some(
        match case.options {
            None => RuleTester::for_component("deterministic-iteration", &component, "rs"),
...
fn an_empty_iteration_scope_is_refused() {
    let Some(component) = component_bytes("deterministic-iteration") else {
        return;
    };
