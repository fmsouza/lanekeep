//! `local/sandbox-containment`, run through the real engine.
//!
...
const RULE_ID: &str = "local/sandbox-containment";
...
const REMEDIATION: &str = "reach the sandbox through `WasmRuntime` and the host context, which is the interface a second engine would implement";
...
///
/// Migrated from `crates/lanekeep-cli/tests/selfcheck.rs`'s `containment` group, updated for the
/// meaning change: the engine is now `wasmtime` in `lanekeep-wasm`, not `rquickjs` in
/// `lanekeep-js`. The harness case there — the `containment()` helper that built a tester — was
/// scaffolding and is not migrated; the four assertions it served are.
const CASES: &[Case] = &[
    // §5.1 keeps every line that knows the engine exists inside lanekeep-wasm.
    Case {
        name: "naming_the_engine_is_reported",
        source: "use wasmtime::Engine;\n",
        options: None,
    ...
    Case {
        name: "a_qualified_use_of_the_engine_is_reported_once",
        source: "fn go() {\n    let c = wasmtime::Engine::new();\n}\n",
        options: None,
        expected: Expected::ReportsAt(&[(2, 13)]),
    },
    // lanekeep-wasm is where the engine is allowed to be named.
    Case {
        name: "the_sandbox_crate_passes",
        source: "use wasmtime::Engine;\n",
        options: Some(r#"{"allow": ["subject/"]}"#),
        expected: Expected::Accepts,
    },
    // Only the engine is contained.
    Case {
        name: "an_unrelated_import_passes",
...
/// `id: "local/sandbox-containment"`, `severity: "warn"` and a remediation reading
...
/// The rule: a WebAssembly component built from `rust-rules/sandbox-containment/`.
///
...
fn component(case: &Case) -> RuleTester {
    const COMPONENT: &[u8] = include_bytes!("../components/sandbox-containment.wasm");
    match case.options {
        None => RuleTester::for_component("sandbox-containment", COMPONENT, "rs"),
        Some(options) => {
            RuleTester::for_component_configured("sandbox-containment", COMPONENT, "rs", options)
        }
    }
    .expect("builds")
}
