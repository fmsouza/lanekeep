//! `local/one-parser-per-file`, run through the real engine.
//!
...
const RULE_ID: &str = "local/one-parser-per-file";
...
const REMEDIATION: &str = "take the tree from the file-level parse — parsing per rule is identical output, N times slower";
...
///
/// Migrated from `crates/lanekeep-cli/tests/selfcheck.rs`'s `one_parser` group. The harness
/// case there — the `one_parser()` helper that built a tester — was scaffolding and is not
/// migrated; the three assertions it served are.
const CASES: &[Case] = &[
    // A parser outside the shared parse means the file is parsed twice.
    Case {
        name: "a_second_parser_is_reported",
        source: "fn go() {\n    let mut parser = tree_sitter::Parser::new();\n}\n",
        options: None,
        expected: Expected::ReportsAt(&[(2, 22)]),
    },
    // Panicking is the failure mechanism in a test, and so is parsing a fixture. The exemption
    // is what the deleted substring test achieved by splitting on `#[cfg(test)]`.
    Case {
        name: "a_parser_in_a_test_module_passes",
        source: "#[cfg(test)]\nmod tests {\n    #[test]\n    fn t() {\n        \
                 let mut parser = tree_sitter::Parser::new();\n    }\n}\n",
        options: None,
        expected: Expected::Accepts,
    },
    // The two real parsers are named in lanekeep.json.
    Case {
...
/// `id: "local/one-parser-per-fil"`, `severity: "warn"` and a remediation reading
...
/// The rule: a WebAssembly component built from `rust-rules/one-parser-per-file/`.
///
...
fn component(case: &Case) -> RuleTester {
    const COMPONENT: &[u8] = include_bytes!("../components/one-parser-per-file.wasm");
    match case.options {
        None => RuleTester::for_component("one-parser-per-file", COMPONENT, "rs"),
        Some(options) => {
            RuleTester::for_component_configured("one-parser-per-file", COMPONENT, "rs", options)
        }
    }
    .expect("builds")
}
