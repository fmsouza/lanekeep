...
fn component(case: &Case) -> RuleTester {
    const COMPONENT: &[u8] = include_bytes!("../components/no-ambient-observation.wasm");
    match case.options {
        None => RuleTester::for_component("no-ambient-observation", COMPONENT, "rs"),
        Some(options) => {
            RuleTester::for_component_configured("no-ambient-observation", COMPONENT, "rs", options)
        }
    }
    .expect("builds")
}
