//! Fold this crate's own sources into the digest `oracle_identity` returns.
//!
//! A directory walk rather than a list of `include_str!` calls: a list is hand-maintained,
//! and a file added but not listed would be a silent gap in the cache key — the exact
//! failure the digest exists to remove.
...
fn main() {
    // Any change under `src/` re-runs this, which is what keeps the digest live.
    ...
    hasher.update(b"lanekeep-types-oracle-v1");
    for file in &files {
    ...
    println!(
        "cargo:rustc-env=LANEKEEP_TYPES_ORACLE_HASH={}",
