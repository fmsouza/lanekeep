...
fn every_committed_artifact_is_the_one_its_sources_build() {
    ...
    let computed = digests(&crate_dir);
    let manifest = crate_dir.join(MANIFEST);
    ...
    if std::env::var_os(BLESS).is_some() {
        std::fs::write(&manifest, render(&computed)).expect("the manifest is writable");
        return;
    }
    ...
    assert!(
        wrong.is_empty(),
        "the committed WebAssembly fixtures are not the ones their sources build:\n\n{}\n\n\
         every test in this crate loads those artifacts, so until they are rebuilt the suite \
         is asserting against binaries that no source in this tree produces. Rebuild and \
         re-record them:\n\n    just wasm-fixtures\n",
        wrong.join("\n")
    );
}
    ...
    text.lines()
        ...
        .map(|line| {
            assert!(
                ...
                "{}: `{line}` is not `<path> <digest>`. This file is written by \
                 `just wasm-fixtures` and is not meant to be edited by hand; re-record it.",
                path.display()
            );
